    ...
    , ui(new Ui::GraphMan)
{
    ...
    connect(ui->kommiBTN, &QPushButton::clicked, this, &GraphMan::kommiBTNclicked);
    ...
    ui->kommiStart->setPlaceholderText("Вершина начала");
    ui->kommiStart->setValidator(v);
...
void branchAndBound(const std::vector<std::vector<long long>>& adjacencyMatrix, std::vector<long long>& path, std::vector<bool>& visited, long long& min_path_cost, std::vector<long long>& min_path, long long& path_cost, long long v, long long level, long long start)
{
    long long n = adjacencyMatrix.size();
    if (level == n && adjacencyMatrix[v][start] != -1)
    {
        // Проверка на наличие пути обратно в начальную вершину
        long long current_path_cost = path_cost + adjacencyMatrix[v][start];
    ...
    for (long long i = 0; i < n; ++i)
    {
        if (!visited[i] && adjacencyMatrix[v][i] != -1)
        {
            visited[i] = true;
            path[level] = i;
            path_cost += adjacencyMatrix[v][i];

            if (path_cost < min_path_cost)
            {
                branchAndBound(adjacencyMatrix, path, visited, min_path_cost, min_path, path_cost, i, level + 1, start);
            }
...
// обвес рекурсивной функции
QString findShortestPath(const std::vector<std::vector<long long>>& adjacencyMatrix, long long start)
{
    long long n = adjacencyMatrix.size();
    // Инициализация пути и минимального расстояния
    std::vector<long long> min_path;
    long long min_path_cost = INT64_MAX;
    std::vector<bool> visited(n, false);
    std::vector<long long> path(n);
    long long path_cost = 0;
    // Начало поиска с начальной вершины
    visited[start] = true;
    path[0] = start;
    branchAndBound(adjacencyMatrix, path, visited, min_path_cost, min_path, path_cost, start, 1, start);
    // Формирование строки с кратчайшим путём
    std::string result;
    if (min_path_cost == INT64_MAX)
    {
        result = "Задача не имеет решения";
    }
    else
    {
        for (unsigned long long i = 0; i < min_path.size(); ++i)
        {
            if (i > 0)
            {
                result += "->";
            }
            result += std::to_string(min_path[i]+1); // поправка на индекс
        }
        result += " (" + std::to_string(min_path_cost) + ")";
    }
    return QString(QString::fromStdString(result));
}

void GraphMan::kommiBTNclicked() // находит кратчайший путь коммивояжера и выводит его
{
    int64_t start = ui->kommiStart->text().toLongLong();
    if(start > static_cast<int64_t>(gr.tableOfvertex.size())-1 || start < 1) return;
    ui->kommiStart->setText("");

    std::vector<std::vector<long long int>> newMatrix = gr.tableOfgraph; //копия матрицы смежности

    for(uint64_t i = 0; i < gr.tableOfgraph.size(); i++) // убираем петли
        for(uint64_t j = 0; j < gr.tableOfgraph.size(); j++)
            if(i == j) newMatrix[i][j] = -1;

    newMatrix.erase(std::next(newMatrix.begin(), 0)); //убираем шапку таблицы
    for (uint64_t i = 0; i < newMatrix.size(); i++) newMatrix[i].erase(std::next(newMatrix[i].begin(), 0)); // убираем нулевой столбец
    ui->output->setText(findShortestPath(newMatrix, start-1)); // -1 поправка на другую индексацию в таблице без шапки
}
